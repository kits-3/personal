...
public class LoanRepositoryImpl implements LoanRepository {
	...
	public List<LoanDto> findByUser(int id) {
		...
		String query = "SELECT l.id,l.loan_date, b.name as bookname, u.name as username FROM loans l join books b on l.book_id = b.id join users u on l.user_id = u.id Where l.user_id = ?";
		try {
			conn = getConnection();
			ps = conn.prepareStatement(query);
			ps.setInt(1, id);
	...
	public int add(Loan loan) {
		String query = "INSERT INTO loans(loan_date, user_id, book_id) values(?,?,?)";
		int result = 0;
		try {
			conn = getConnection();
			ps = conn.prepareStatement(query);
	...
	@Override
	public int deActivate(int bookId) {
		String query = "UPDATE loans SET isactive=0 where book_id = ?";
		int result = 0;
		try {
			conn = getConnection();
			ps = conn.prepareStatement(query);
