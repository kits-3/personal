...
public class BookRepositoryImpl implements BookRepository {
	...
	public void add(Book book) {
		String query = "INSERT INTO books(name,author_id,publisher_id,available) values(?,?,?,?)";
		try {
			conn = getConnection();
			ps = conn.prepareStatement(query);
	...
	public void update(Book book) {
		String query = "UPDATE books SET name = ?, author_id =?, publisher_id =?, available=? where id = ?";
		try {
			conn = getConnection();
			ps = conn.prepareStatement(query);
			ps.setString(1, book.getName());
