...
public class TaskReopsitoryImpl implements ITaskRepository{
	...
	public boolean isCheckUpdateTaskByStatusOrDisplayOrDescription(int idTask, int status, int display,
		...
		try {
			...
			if (count > 0) {
				isSuccess = true;
				if (display == 0) {
					queryString = "Select * from taskdetail where task_id = ?";
					stmt = connection.prepareStatement(queryString);
	...
	@Override
	public String getNameTaskByIdTask(int idTask) {
		String getTaskName = "";
		try {
			...
			int index = 0;
			while(rs.next()) {
				getTaskName = rs.getString("header");
				index++;
			}
			if (index == 0) {
				getTaskName = "";
	...
	@Override
	public ArrayList<TaskEntity> getAllTaskByIdUserDepisteStatusOrDisplay(int idUser) {
		ArrayList<TaskEntity> lsTask = new ArrayList<TaskEntity>();
		try {
			String queryString = "SELECT * FROM task WHERE OWNER = ?";
			connection = getConnection();
			stmt = connection.prepareStatement(queryString);
			stmt.setInt(1, idUser);
			rs = stmt.executeQuery();
