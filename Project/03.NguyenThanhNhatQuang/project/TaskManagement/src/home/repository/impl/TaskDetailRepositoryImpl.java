...
public class TaskDetailRepositoryImpl implements ITaskDetailRepository {
	...
	public ArrayList<TaskDetailEntity> getListTaskDetailByIdTask(int idTask) {
		...
		try {
			String queryString = "SELECT * FROM taskdetail WHERE task_id = ? and display = 1";
	...
	public TaskDetailEntity taskDetail(int idTaskDetail) {
		...
		try {
			String queryString = "select * from taskdetail where id = ? and display = 1";
			connection = getConnection();
			stmt = connection.prepareStatement(queryString);
			...
			while(rs.next()) {
				...
				taskDetail.setTask_id(rs.getInt("task_id"));
	...
	@Override
	public ArrayList<TaskDetailEntity> getAllTaskDetail() {
		ArrayList<TaskDetailEntity> listTaskDetail = new ArrayList<TaskDetailEntity>();
		try {
			String queryString = "SELECT * FROM taskdetail";
			connection = getConnection();
			stmt = connection.prepareStatement(queryString);
			rs = stmt.executeQuery();
			int index = 0;
