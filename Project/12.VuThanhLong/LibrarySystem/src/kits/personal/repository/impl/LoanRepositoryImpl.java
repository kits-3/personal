...
public class LoanRepositoryImpl implements LoanRepository {
	...
	public int update(Loan loan) {
		String query = "UPDATE loans SET loan_date =?, isactive=?, user_id=?, book_id=?, return_date = ? where id = ?";
		int result = 0;
		try {
			conn = getConnection();
			ps = conn.prepareStatement(query);
	...
	public int deActivate(int bookId,Date returnDate) {
		String query = "UPDATE loans SET isactive = 0, return_date = ? where book_id = ? and isactive = 1";
		int result = 0;
		try {
			conn = getConnection();
			ps = conn.prepareStatement(query);
