...
public class UserServiceImpl implements UserService {
	...
	public void signUp() {
		...
		while (result == 0) {
			...
			if (isContinue("sign up again").equalsIgnoreCase("NO"))
				return;
		}

	}

//	CHECK ACCOUNT'S EXPIRATION
	@Override
	...
	public void showUserMenu() {
		...
		System.out.println("1. Show available books and borrow");
...
//	SEARCH BOOK BY INFORMATION(AUTHOR, PUBLISHER, BOOK NAME)
	@Override
	...
	public List<BookDto> showBorrowBook(int userId) {
		...
		for (BookDto item : list) {
			...
			if (diffDays > 30) {
				...
				Loan loan = loanRepository.findById(item.getLoanId());
...
//	BORROW BOOK
	...
	public void returnBook(int id) {
		...
		while (result == 0) {
			if (showBorrowBook(id).isEmpty()) {
				System.out.println("You don't borrow any book");
				return;
			}

			if (isContinue("return book").equalsIgnoreCase("NO"))
				return;
			...
			result = loanRepository.deActivate(bookId, getCurrentDate());
			if (result == 0) {
				System.out.println("Book id was wrong. Please choose again");
			}
	...
	public int updateUser(User user) {
		...
		while (result == 0) {
			try {
				if (isContinue("update information").equalsIgnoreCase("NO"))
					break;
	...
	public void saveBorrow() {
		String filedir = "C:/Users/PC12/Desktop/me/LibrarySystem/history/history.txt";
	...
	public void saveFeeHistory() {
		String filedir = "C:/Users/PC12/Desktop/me/LibrarySystem/history/fee.txt";

		try {
			FileOutputStream fo = new FileOutputStream(filedir);
			String head = "`````````Transaction history````````````\nID\t\tFee name\t\tUsername\tMount\t\tDate\n";
			String detail = "";
			byte[] bytes = head.getBytes();
			fo.write(bytes);
