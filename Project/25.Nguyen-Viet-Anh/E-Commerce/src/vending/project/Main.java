...
public class Main {
	...
	// 미리 객체를 만들어놓은 이유는 ? ->  creat flow 
	//	static User user = new User(); <-- so no need this
	...
	public static void showMenuUser(User user) { //<-- where called this
		System.out.println("--------------------------------------------");
	...
	public static void showLogin() {
		...
		System.out.println("------------------------------------------");
		System.out.print("Your choose: ");
		String choose = sc.next(); sc.nextLine();
		switch (choose) {
		case "1":
			user = userImpl.login();
			if( user == null ){
				System.out.println("------------------------------------------");
				System.out.println("Cannot login please check your info ~");
				showLogin();
			}
			else {
	...
	public static void addToCart(User user) {
		...
		orderItemsList.add(orderICart);
		order.setOrderItems(orderItemsList);
		order.setUser(user);
		newCart = true;
	}
	
	public static void viewCart(Product pro,User user) {
		int total = 0;
		boolean stop = false;
		System.out.println("------------------------------------------");
